...
path_c_code = os.path.join(os.path.dirname(os.path.abspath(__file__)), "c_code/")
call("gcc -shared -Wl,-soname,run_simulation -o run_simulation.so -fPIC run_simulation.c", cwd=path_c_code, shell=True)
run_sim = ct.cdll.LoadLibrary(os.path.join(path_c_code, 'run_simulation.so'))
run_sim.check_import()
"""Specification of the object types of input and output of the C function run_sim.sim()"""
run_sim.sim.argtypes = [np.ctypeslib.ndpointer(dtype = np.bool_),
...
def execute_c_code(syn, patt, **kwargs):
    ...
    **kwargs :
    ...
    """
    # Creating arguments in adapted formats:
    I = syn.I.copy().astype(np.bool_)
    S = syn.S.copy().astype(np.bool_)
    nr, nc = I.shape
    spat = patt.spat.copy()
    nit = len(spat)
    tstep = patt.tstep
